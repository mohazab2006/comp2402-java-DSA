 ...
 * @param <T>
 */
public class SkipItSlow<T> implements IndexedSSet<T> {
	...
	@SuppressWarnings("unchecked")
	public SkipItSlow(Comparator<T> c) {
		this.c = c;
		n = 0;
	...
	public void deleteSpan(int i, int j){
		// Check out-of-bounds conditions with the indices
		if (i > j) {
			int temp = i;
			i = j;
			j = temp;
		}
		if ((i < 0 && j < 0) || (i > n-1)) return;
		if (i < 0)
			i = 0;
		if (j > n-1)
			j = n-1;
